import { IRemedyCalendarDay } from "./Interface/Interface";
...
const yearCalendarCache = new Map<string, any>();

// Helper pour obtenir le premier jour de la semaine
function getFirstDayOfWeek(calendarStyle: 'eu' | 'am'): number {
  return calendarStyle === 'eu' ? 1 : 0; // EU: Lundi=1, AM: Dimanche=0
}

// Helper pour formater les noms de jours/mois - OPTIMISÉ
function getFormat(date: Date, type: 'dayName' | 'dayShort' | 'monthName' | 'monthYear', localeCode: localeCode): string {
...
function getWeekBounds(date: Date, firstDayOfWeek: number): { start: Date; end: Date } {
...
function generateWeekDays(weekStart: Date, monthStart: Date, localeCode: localeCode): IRemedyCalendarDay[] {
...
function generateMonthWeeks(monthStart: Date, localeCode: localeCode, calendarStyle: 'eu' | 'am') {
...
export const limitCacheSize = (maxSize: number = 2000) => {
...
export const limitYearCacheSize = (maxYears: number = 3) => {
